...
// Displays all the categories	
	...
	public ModelAndView categoryList(@ModelAttribute("customer") Customer customer, HttpSession session) {
		...
		if(null == sesOrderForm.getCustomer() || (customer.getId()!=sesOrderForm.getCustomer().getId()&&customer.getId()>0)){
	...
	// Displays list of products for the selected category in the home page
	@RequestMapping(value = "/displayProductsForm", method = RequestMethod.GET)
	public ModelAndView displayProductsForm(@RequestParam(value="catId", required = true) int catId, HttpSession session) {
		ModelAndView modelView;
	...
	// Takes the list of products selected, their quantities and update the Order object in session
